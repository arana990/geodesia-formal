...
theorem fluxo_acumula_a_fonte (y₆ ρy₁ : ℝ → ℝ) (x : ℝ) (hx : x ≠ 0)
...
theorem potencial_menos_massa_e_constante (y₅ M ρy₁ : ℝ → ℝ) (x : ℝ) (hx : x ≠ 0)
...
theorem constante_no_aberto (a : ℝ) (F : ℝ → ℝ)
...
theorem constante_e_zero (a : ℝ) (ha : 0 < a) (F : ℝ → ℝ)
...
theorem potencial_iguala_a_massa_encerrada (a : ℝ) (ha : 0 < a) (y₅ M ρy₁ : ℝ → ℝ)
  ...
  have hzero : ∀ z ∈ Ioo (0:ℝ) a, HasDerivAt (fun t => t * y₅ t - t * M t) 0 z :=
    fun z hz => potencial_menos_massa_e_constante y₅ M ρy₁ z (ne_of_gt hz.1)
      (hM z hz) (h5 z hz)
  have hc := constante_e_zero a ha _
    (fun u hu v hv => constante_no_aberto a _ hzero u v hu hv) hreg x hx
...
está em `potencial_iguala_a_massa_encerrada`**, que prova aquela igualdade a partir do
...
theorem k_carga_do_grau_zero_e_nulo (y₅ M : ℝ → ℝ) (x : ℝ)
